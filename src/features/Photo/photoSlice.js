...
// var findIndex = (photo, id) => {
//     var result = -1;
//     photo.forEach((pt, index) => {
//         if (pt.id === id) {
//             result = index
//         }
//     });
...
const photo = createSlice({
    ...
    reducers: {
        ...
        onRemove: (state, action) => {
            ...
            //Tạo state hiện tại ra một mảng mới nên mới cần return
            return state.filter(photo => photo.id !== photoId)
            //Suwrdungj state hieenjtaij thì ko cần return
        },
        onEdit: (state, action) => {
            const newPhoto = action.payload;
            const photoIndex = state.findIndex(x => x.id === newPhoto.id);
            if (photoIndex >= 0) {
...
export const { addPhoto, onRemove, onEdit } = actions;
